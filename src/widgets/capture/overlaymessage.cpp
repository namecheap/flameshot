...
void OverlayMessage::setActive(OverlayMessage* message)
{
    // With one capture widget per display, each owns an overlay but only the
    // armed display's may receive messages.
    if (message != nullptr) {
        m_instance = message;
    }
}
